...
class TestStudyLLM:
    ...
    def test_init_with_invalid_path(self):
        ...
        with patch('llm.RAGHelper', side_effect=ValueError("Error initializing RAGHelper: test error")):
            with pytest.raises(ValueError, match="Error initializing RAGHelper: test error"):
    ...
    def test_init_with_rag_helper_error(self, temp_pdf_file):
        ...
        with patch('llm.RAGHelper', side_effect=Exception("RAGHelper initialization failed")):
            with pytest.raises(ValueError, match="Error initializing RAGHelper: RAGHelper initialization failed"):
                StudyLLM(temp_pdf_file)

    @pytest.mark.asyncio
    async def test_prompt_success(self, mock_rag_helper, mock_chat_llm, temp_pdf_file):
        """Test successful prompt processing"""
        with patch('llm.RAGHelper', return_value=mock_rag_helper), \
             patch.object(StudyLLM, 'chat_llm', mock_chat_llm):
            
            llm = StudyLLM(temp_pdf_file)
            
            result = await llm.prompt("What is the main topic?")
            
            assert result.content == "Mock chat response"
            mock_rag_helper.retrieve.assert_called_once_with("What is the main topic?", top_k=5)
            mock_chat_llm.ainvoke.assert_called_once()

    @pytest.mark.asyncio
    async def test_prompt_with_context_filtering(self, mock_rag_helper, mock_chat_llm, temp_pdf_file):
    ...
    @pytest.mark.asyncio
    async def test_prompt_rag_error(self, mock_rag_helper, mock_chat_llm, temp_pdf_file):
    ...
    @pytest.mark.asyncio
    async def test_prompt_llm_error(self, mock_rag_helper, mock_chat_llm, temp_pdf_file):
        """Test prompt handling when LLM fails"""
        mock_chat_llm.ainvoke.side_effect = Exception("LLM processing failed")
        
        with patch('llm.RAGHelper', return_value=mock_rag_helper), patch.object(StudyLLM, 'chat_llm', mock_chat_llm):
    ...
    @pytest.mark.asyncio
    async def test_summarize_success(self, mock_rag_helper, mock_generation_llm, temp_pdf_file):
        ...
        with patch('llm.RAGHelper', return_value=mock_rag_helper), \
             patch('llm.StudyLLM.generation_llm', mock_generation_llm), \
    ...
    @pytest.mark.asyncio
    async def test_summarize_chain_error(self, mock_rag_helper, mock_generation_llm, temp_pdf_file):
    ...
    @pytest.mark.asyncio
    async def test_generate_flashcards_success(self, mock_rag_helper, mock_generation_llm, temp_pdf_file):
        ...
        with patch('llm.RAGHelper', return_value=mock_rag_helper), \
            ...
            mock_flashcard_chain.invoke.assert_called_once_with(mock_rag_helper.summary_chunks)

    @pytest.mark.asyncio
    async def test_generate_flashcards_error(self, mock_rag_helper, mock_generation_llm, temp_pdf_file):
        """Test flashcard generation when chain fails"""
        mock_flashcard_chain = AsyncMock()
        mock_flashcard_chain.invoke.side_effect = Exception("Flashcard generation failed")
    ...
    @pytest.mark.asyncio
    async def test_generate_quiz_success(self, mock_rag_helper, mock_generation_llm, temp_pdf_file):
        ...
        with patch('llm.RAGHelper', return_value=mock_rag_helper), \
            ...
            mock_quiz_chain.invoke.assert_called_once_with(mock_rag_helper.summary_chunks)

    @pytest.mark.asyncio
    async def test_generate_quiz_error(self, mock_rag_helper, mock_generation_llm, temp_pdf_file):
    ...
    @pytest.mark.asyncio
    async def test_multiple_prompt_calls(self, mock_rag_helper, mock_chat_llm, temp_pdf_file):
    ...
    @pytest.mark.asyncio
    async def test_context_retrieval_parameters(self, mock_rag_helper, mock_chat_llm, temp_pdf_file):
    ...
    @pytest.mark.asyncio
    async def test_task_description_in_prompt(self, mock_rag_helper, mock_chat_llm, temp_pdf_file):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_operations(self, mock_rag_helper, mock_chat_llm, mock_generation_llm, temp_pdf_file):
        ...
        with patch('llm.RAGHelper', return_value=mock_rag_helper),             patch('llm.load_summarize_chain', return_value=mock_summary_chain):
