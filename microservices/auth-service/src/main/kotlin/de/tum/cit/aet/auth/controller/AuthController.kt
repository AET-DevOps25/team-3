...
class AuthController(
    ...
    fun register(
        ...
        logger.info("POST /auth/register - username: {}", request.username)
    ...
    fun getCurrentUser(authentication: Authentication): ResponseEntity<Any> {
        logger.info("GET /auth/me - user: {}", authentication.name)
    ...
    fun updateCurrentUser(
        ...
        logger.info("PUT /auth/me - user: {}", authentication.name)
        ...
        return try {
            val response = authService.updateUser(authentication.name, request)
            ResponseEntity.ok(response)
        } catch (e: IllegalArgumentException) {
            ResponseEntity.badRequest().body(
                AuthErrorResponse(
    ...
    fun changePassword(
        ...
        logger.info("PUT /auth/me/password - user: {}", authentication.name)
        ...
        return try {
            val success = authService.changePassword(authentication.name, request)
            if (success) {
            ...
            logger.error("Password change error: {}", e.message)
            ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
                AuthErrorResponse(
                    error = "Password change failed",
                    message = "An internal error occurred"
    ...
    @PreAuthorize("hasRole('ADMIN')")
    fun searchUsers(@ModelAttribute request: UserSearchRequest): ResponseEntity<Any> {
        logger.info("GET /auth/users/search")
    ...
    @PreAuthorize("hasRole('ADMIN')")
    fun deactivateUser(@PathVariable id: String): ResponseEntity<Any> {
        logger.info("POST /auth/users/{}/deactivate", id)
        
        return try {
            val success = authService.deactivateUser(id)
            if (success) {
                ResponseEntity.ok(mapOf("message" to "User deactivated successfully"))
            } else {
                ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
                    AuthErrorResponse(
            ...
            logger.error("Deactivate user error: {}", e.message)
            ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
                AuthErrorResponse(
                    error = "Deactivation failed",
                    message = "An internal error occurred"
