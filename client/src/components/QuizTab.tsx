  ...
  useEffect(() => {
    ...
    const fetchQuizzes = async () => {
      const quizPromises = newDocumentIds.map(async (documentId) => {
        try {
          // First, check if quiz data is already available from automatic processing
          ...
          if (documentContent.status === 'PROCESSING' || 
            ...
            return {
              ...
              documentId,
              status: 'GENERATING',
              error: 'Quiz is being generated by automatic processing...'
            };
          }
          
          // For recently uploaded documents where automatic processing hasn't set quiz status yet,
          // wait for automatic processing instead of making individual API calls
          if ((documentContent.summaryStatus as string) === 'UPLOADED' || 
              (documentContent.quizStatus as string) === 'UPLOADED') {
            return {
              questions: [],
              documentName: documentContent.originalName || 'Unknown Document',
              documentId,
          ...
          // but failed to generate quiz (summary is READY/ERROR but quiz is ERROR)
          if ((documentContent.quizStatus as string) === 'ERROR' || 
              ((documentContent.summaryStatus as string) === 'READY' && (documentContent.quizStatus as string) === 'UPLOADED')) {
            ...
            if (res.status === 'GENERATING') {
              ...
              return {
                questions: [],
                documentName: res.documentName || 'Unknown Document',
                documentId,
                status: 'GENERATING',
                error: res.error || 'Quiz is being generated...'
              };
            }
